...
func New(cfg config.Config, userStore store.AdminStore, walletopsClient *walletops.Client, gmrEngineClient *gmrengine.Client, newsAgent *newsagent.Agent, sessions session.Manager) *fiber.App {
	...
	pilotEventRateLimit := limiter.New(limiter.Config{Max: 40, Expiration: time.Minute})
	pilotFeedbackRateLimit := limiter.New(limiter.Config{Max: 8, Expiration: 10 * time.Minute})
	...
	api.Post("/pilot/events", pilotEventRateLimit, server.recordPilotEvent)
	api.Post("/pilot/feedback", pilotFeedbackRateLimit, server.createPilotFeedback)
	...
	admin.Get("/pilot", server.adminPilotOverview)
	admin.Patch("/pilot/feedback/:id", adminMutationRateLimit, server.adminUpdatePilotFeedback)
